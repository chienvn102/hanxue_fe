        ...
        <div>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                <div>
                    ...
                    <p className="text-gray-500 text-sm">Tổng cộng {total.toLocaleString()} từ vựng</p>
                </div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                {loading ? (
                    ...
                    <div className="p-8 text-center text-gray-500">Không tìm thấy từ vựng nào.</div>
                ) : (
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {vocabs.map(vocab => (
                                <tr key={vocab.id} className="hover:bg-gray-50 transition-colors">
                                    <td className="px-4 py-3 text-xs text-gray-400 font-mono">{vocab.id}</td>
                                    <td className="px-4 py-3 text-xl font-bold text-gray-900">{vocab.simplified}</td>
                                    <td className="px-4 py-3 text-sm text-[var(--primary)]">{vocab.pinyin}</td>
                                    <td className="px-4 py-3 text-sm text-gray-500 italic">{vocab.hanViet}</td>
                                    <td className="px-4 py-3 text-sm text-gray-700 max-w-xs truncate">{vocab.meaningVi}</td>
                                    ...
                                    <td className="px-4 py-3 text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            <button onClick={() => openEditModal(vocab)} className="p-1.5 text-gray-400 hover:text-blue-500 transition-colors">
                                                <Icon name="edit" size="sm" />
                                            </button>
                                            <button onClick={() => handleDelete(vocab.id)} className="p-1.5 text-gray-400 hover:text-red-500 transition-colors">
                    ...
                    <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl my-8">
                        ...
                        <form onSubmit={handleSave} className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">Chữ giản thể *</label>
                                    <input
                                        type="text" required
                                        className="w-full px-3 py-2 border rounded-lg bg-white text-gray-900 text-2xl font-bold"
                                        value={formData.simplified}
                                        onChange={e => setFormData({ ...formData, simplified: e.target.value })}
                                    />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">Chữ phồn thể</label>
                                    <input
                                        type="text"
                                        className="w-full px-3 py-2 border rounded-lg bg-white text-gray-900 text-2xl font-bold"
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">Hán Việt *</label>
                                    <input
                                        type="text" required
                                        className="w-full px-3 py-2 border rounded-lg bg-white text-gray-900 italic"
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Nghĩa tiếng Anh</label>
                                <input
                                    type="text"
                                    className="w-full px-3 py-2 border rounded-lg bg-white text-gray-900"
                            ...
                            <div>
                                ...
                                <div className="flex flex-col gap-2">
                                    <div className="flex gap-2">
                                        <label className={`flex-1 px-3 py-2 border-2 border-dashed rounded-lg cursor-pointer transition-colors ${uploading ? 'bg-gray-100 border-gray-300' : 'border-gray-300 hover:border-[var(--primary)] hover:bg-[var(--primary)]/5'}`}>
                                            <input
                                                type="file"
                                                accept="audio/*"
                                                className="hidden"
                                        ...
                                        {formData.audio_url && (
                                            <button
                                                ...
                                                className="px-4 py-2 bg-green-100 text-green-600 rounded-lg hover:bg-green-200 transition-colors flex items-center gap-1"
                                            >
                                    ...
                                    {formData.audio_url && (
                                        <div className="flex items-center gap-2 text-xs">
                                            <span className="text-gray-500 truncate flex-1">{formData.audio_url}</span>
                                            <button
                                                type="button"
