...
function CourseCard({ course, isLocked }: { course: Course; isLocked: boolean }) {
    ...
    const cardContent = (
        <article className={`relative bg-[var(--surface)] rounded-2xl border border-[var(--border)] overflow-hidden transition-all duration-300 ${
            isLocked
                ? 'opacity-60 cursor-not-allowed'
                : 'hover:border-[var(--primary)]/50 hover:shadow-xl hover:shadow-[var(--primary)]/5'
        } ${isInProgress ? 'ring-2 ring-[var(--primary)]/30' : ''}`}>
            {/* Background gradient */}
            ...
            <div className="relative p-6">
                ...
                <div className="flex items-start justify-between mb-4">
                    ...
                    {isLocked && (
                        <div className="w-8 h-8 rounded-full bg-[var(--surface-secondary)] flex items-center justify-center">
                            <Icon name="lock" size="sm" className="text-[var(--text-muted)]" />
                        </div>
                    )}
                </div>

                {/* Title & Description */}
                <h2 className={`text-xl font-bold text-[var(--text-main)] mb-2 ${!isLocked ? 'group-hover:text-[var(--primary)]' : ''} transition-colors`}>
                    {course.title}
                </h2>
                        ...
                        <div className="flex justify-between text-xs">
                            ...
                            <span className={
                                isCompleted ? 'text-emerald-500 font-medium'
                                    : isInProgress ? 'text-[var(--primary)] font-medium'
                                    : isLocked ? 'text-[var(--text-muted)]'
                                    : 'text-[var(--text-muted)]'
                            }>
                                {isCompleted ? 'Hoàn thành'
                                    : isInProgress ? `Đang học: ${progress}%`
                                    : isLocked ? 'Chưa mở'
                                    : 'Chưa bắt đầu'}
                            </span>
                        </div>
                            ...
                            <div
                                className={`h-full rounded-full transition-all duration-500 ${
                                    isCompleted ? 'bg-emerald-500'
                                        : 'bg-gradient-to-r from-[var(--primary)] to-orange-500'
                                }`}
                ...
                {isLocked ? (
                    ...
                    <button className="mt-4 w-full py-2.5 rounded-xl bg-[var(--primary)] text-white font-medium text-sm hover:bg-[var(--primary-hover)] transition-colors flex items-center justify-center gap-2">
